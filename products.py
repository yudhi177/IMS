...
class productClass:
    ...
    def add(self):
        ...
        cur = con.cursor()
        try:
            if self.var_name.get() == "Select" or self.var_cat.get()=="Empty" or self.var_name.get()=="":
                messagebox.showerror("Error", "All fields are required", parent=self.root)
            else: 

                # Generate a unique product ID (replace with your preferred method)
                # Here's an example using a timestamp approach:
                import time
            self.var_pid.set(time.strftime('%Y%m%d%H%M%S'))

            cur.execute("INSERT INTO product (pid, Category, Supplier, name, price, qty, status) VALUES (?, ?, ?, ?, ?, ?, ?)", (
                    self.var_pid.get(),  # Use the generated pid value
                    self.var_cat.get(),
                    self.var_sup.get(),
                    self.var_name.get(),
                    self.var_price.get(),
...
# Update table display
    ...
    # Use the retrieved product details to populate the entry fields (corrected order)
        self.var_pid.set(row[0])  # Assuming the first column is PID
        self.var_sup.set(row[1])  # Assuming the second column is supplier
        self.var_cat.set(row[2])  # Assuming the third column is category
        self.var_name.set(row[3])  # Assuming the fourth column is name
        self.var_price.set(row[4])  # Assuming the fifth column is price
        self.var_quantity.set(row[5])  # Assuming the sixth column is quantity
        self.var_status.set(row[6])  # Assuming the seventh column is status
    ...
    def search(self):
        ...
        try:
            if self.var_searchby.get()=="Select":
                ...
                cur.execute("select * from Product WHERE {} LIKE ?".format(self.var_searchby.get()), (search_term,))
                rows=cur.fetchall()
